...
// User actor is responsible for handling all the user related requests for the userService class

class UserActor extends Actor {
  val userService = new UserService()

  def receive = {
    case AddUser(nationalId, name) =>
      userService.addUser(nationalId, name)
    case RemoveUser(nationalId) =>
      userService.removeUser(nationalId)
    case UpdateUser(nationalId, name) =>
      userService.updateUser(nationalId, name)
    case GetAllUsers() =>
      userService.getAllUsers()
    case GetUserByNationalId(nationalId) =>
      userService.getUserByNationalId(nationalId)
  }

}

case class AddUser(nationalId: String, name: String)
case class RemoveUser(nationalId: String)
case class UpdateUser(nationalId: String, name: String)
case class GetAllUsers()
case class GetUserByNationalId(nationalId: String)
